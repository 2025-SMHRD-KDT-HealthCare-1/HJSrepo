...
const Ex05map = () => {
    ...
    // 1, filter 함수 사용
    const filterdList = numList.filter(num => num % 2 === 0);
  ...
  return (
    <div>
        ...
        */}
        {students.map((item) => (
            <p key={item.name}>
                안녕하세요 제 이름은 {item.name}입니다 저는 {item.bloodType}이고. {item.age}살입니다
            </p>
         ...
         */}
            ...
            {food.map((menu, index) => (
                <button style={{background : 'yellow'}}key={index}>{menu}</button>
            ))}
         ...
         */}
            <br></br>
            {numList.map((unm, index)=>(
                <button style={{background : 'red'}} key={index}>{unm*2}</button>
            ))}

            <h3>Case 4 : filter함수 실습</h3>
            {/* 
                numList배열을 map함수를 통해서 => 버튼요소로 만들기 
